...
<body>
    <nav class="navbar">
        ...
        <a href="index.php">< Enregistrer Un Participant /></a>
        <a href="#">< Voir la Liste des Participants /></a>
    </nav>
    <main>
        <h1 class="titre">< Les Participants /></h1>

        <form method="POST" class="formulaire">
            <label for="activite">Sélectionnez une activité :</label>
            <div class="selection">
                <select name="activite_id" id="activite">
                    <option value="all" <?php if ($selectedActiviteId == "all") echo 'selected'; ?>>Toutes les activités</option>
                    <?php foreach ($activites as $id => $intitule) { ?>
                        <option value="<?php echo $id; ?>" <?php if ($selectedActiviteId == $id) echo 'selected'; ?>>
                            <?php echo $intitule; ?>
                        </option>
                    <?php } ?>
                </select>
                ...
                <tbody>
                    <?php foreach ($participants as $participant) { ?>
                        <tr>
                            ...
                            <?php if ($selectedActiviteId == "all") { ?>
                                <td><?php echo $participant['activite']; ?></td>
                            <?php } ?>
                        </tr>
                    <?php } ?>
                </tbody>
